...
- **What it covers** (one worker, serialised, ~6 min cold): `e2e/theme.spec.ts` (the light/dark preference survives navigation, reload and every later mount of the toggle, with the OS scheme emulated both ways), `e2e/visibility.spec.ts` (a document's visibility control agrees with the stored row in both directions, on `/files` and on the client Files tab, and a failed change says so instead of looking like a silent revert), and `e2e/visual.spec.ts` (§ `UI.md` 10.14 — 42 stops × light/dark × desktop/mobile = 168 screenshots to `.design-shots/` (written locally only — CI skips the file write, nothing there reads them), plus a per-stop DOM audit: one `h1`, no invisible text, no untranslated keys, no horizontal overflow at 390 px, no console errors or failed requests).
...
- **CI:** the `e2e` job in `.github/workflows/ci.yml` — runs after `check` **and** `isolation`, brings up its **own `postgres:18` service container**, migrates it from empty, seeds it, caches `~/.cache/ms-playwright`, installs chromium, runs the suite, force-cleans the fixture, and uploads `playwright-report/` **and `test-results/`** on failure **or cancellation** (a job killed by `timeout-minutes` is cancelled, not failed — and the HTML report is only built when a run ends, so `test-results/` is the part that survives a kill). It is **blocking**: if it starts flaking, add `continue-on-error: true` to that job rather than deleting it. *(Amended 2026-09-01. The `isolation-db` concurrency group is **gone** — it existed only because this job and the isolation suite shared one dev database. The `needs: [check, isolation]` ordering stays, and it is now a **cost** control rather than a data one: a red isolation run skips this job, so a failing push pays for one db job instead of two. Under CI the suite no longer writes the 164 `.design-shots/` PNGs — nothing reads them there — while every DOM/craft/console assertion still runs.)*
