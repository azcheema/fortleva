...
- **Pre-flight checks (CI runs the same):** `pnpm typecheck` · `pnpm lint --max-warnings 0` · `pnpm test` · `pnpm test:db` (against a real Postgres as `app_runtime`) · `pnpm build`.
...
## 7. Incident quick reference *(TODO)*
