...
### ARC-09 · Transactional email — Resend (behind a thin adapter), with EU honesty

- **Decision.** **Resend** as the v1 ESP for invites, notifications, continuity-box alerts, and billing emails — behind a one-interface mail adapter (`send(message)`) so the vendor is swappable in a day. DKIM/SPF on the product domain (ARC-11). Policy: **emails carry links, not data** — deep links to the canonical app domain (no tenant slugs, §3), no invoice PDFs or sensitive contents attached; this both minimizes personal data at the ESP and serves the seam rules.
- **Rationale.** Best DX for this stack (React Email templates, first-class Next/Vercel integration), free tier covers v1 volume, and transactional email is inherently *not* a residency-controllable channel — the message leaves EU control at the recipient's mail server regardless of ESP. The honest posture is therefore: minimal personal data in bodies, ESP listed as a sub-processor with DPA + SCCs in `SECURITY.md`.
- **EU consideration & verification item.** Resend is a US company; we have **not verified** a current EU-residency option — verify DPA/SCC terms and any EU data-location option at integration time (tracked in `OPEN_QUESTIONS.md`). If a tenant contract demands EU-only processors for email, the adapter swaps to AWS SES in `eu-central-1`/`eu-north-1` or an EU ESP (Scaleway TEM, Brevo) — contained change by design.
- **Rejected & why.** *AWS SES directly in v1* — cheapest at scale but adds an AWS account, sandbox escalation, and template plumbing now for volume we don't have. *Postmark/SendGrid* — fine, no EU advantage, weaker DX here. *SMTP from the app* — deliverability suicide.
- **Revisit when.** Volume passes ~50k/mo, an EU-only-processor contract lands, or Resend's residency options change.
...
| ESP (Resend-class) | $0 | ~$20 | volume is tiny; verify tier at integration |
...
- DPA/sub-processor honesty (Vercel/Neon/Cloudflare/Stripe/ESP US-transfer notes), encryption, signed-URL policy: `SECURITY.md`.
- Feeding `OPEN_QUESTIONS.md`: Neon Frankfurt project creation (blocks Phase 1); product-domain purchase (**no longer blocks Phase 1** — deferred to Phase 7 by decision #9, B1); SEK/USD currency split (blocks Phase 7); ESP DPA/EU verification; Vercel WAF EU usage rates; Idura contract terms at v1.5.
