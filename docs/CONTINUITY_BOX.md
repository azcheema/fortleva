...
**Amended 2026-08-16 (decision 12 — the vault is a separate module; the box stays pointer-only).** The work-management plan adds a **credential vault + asset registry** as its own entitlement-gated product module (`vault`, Phase 3V — `CredentialItem`/`CredentialSecret`, `CredentialShareLink`, `ClientAsset`, expirations feed) with **server-side envelope encryption** (`TenantKey` per tenant, v2 ciphertext with AAD; `ARCHITECTURE.md` ARC-20, `SECURITY.md` §6). That is a *different* object from this box and does not reopen §6: the box remains **pointer-first, browser-encrypted, zero-knowledge to the Platform**, and none of §1–§5, §7–§13 changes. Two touchpoints only. (1) **At seal time the ceremony auto-generates the "systems & assets" part of the template (rows 1–4 above) from `ClientAsset`** — non-secret metadata: type, provider, identifier, account owner, expiry/renewal dates, criticality — pre-filled into the checklist for the Sealing Member to edit; and row 5 ("Secrets & environment") is pre-filled with **where the credentials live**: the Fortleva vault for this Client (item names and count, never values) or, if the tenant keeps its own manager, that pointer as today. The generated text is still sealed inside the ciphertext; DB metadata learns nothing new. (2) **A vault export can be included in the sealed package only by an explicit, separate member action** during the ceremony (`credential:export`, ✦ step-up, audited `credential.exported` + `continuity_box.sealed` metadata flag `includesVaultExport=true`) — never by default and never silently; the §6 credential-discouragement warning is shown at that step, and the box then carries a *snapshot* that rots exactly as point 1 (Rot) above predicts, which is why the default stays "point at the vault". Phase 8 depends on Phase 3V only for the auto-fill; if 3V has not shipped, the section is a manual checklist as before.
