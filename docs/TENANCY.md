...
- **Portal forbidden-columns grep** on `src/modules/*/portal.ts`: no select/return of rates, cost, `internalNotes`, `repoUrl`, `hostingNotes`, non-billable seconds, per-member breakdowns, state *names*, labels, links, `assigneeMemberId`, INTERNAL activity rows, ciphertext columns; the allow-list per projection is explicit and the grep fails on anything outside it.
