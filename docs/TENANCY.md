...
**Amended 2026-08-16 (work-management plan, decisions 11–13).** The tenant-owned set grows by the Work / Time / Vault / notification / search tables (WorkItem, WorkflowState, WorkflowPreset, WorkItemActivity, WorkItemCollaborator, Label, Comment, Mention, ProjectUpdate, ProjectUpdateInternalSnapshot, TimeEntry, RateCard, ProjectBudget, BudgetAlert, ProjectTimeSummary, StaffNotice(+Acknowledgment), TenantKey, CredentialItem, CredentialSecret, CredentialVersion, CredentialShareLink, ClientAsset, Notification, Subscription, NotificationPreference, EmailOutbox, EmailSuppression, `search_index`, TenantCounter — DATA_MODEL.md §6.14–§6.19). ~~Issue, IssueComment~~ are **superseded** by `WorkItem(kind=REQUEST)` and the polymorphic `Comment`. The model registry (`src/db/model-registry.ts`) now classifies every tenant-owned model into one of three **subclasses**, each with its own posture test (§7.2, §11):

| Subclass | Meaning | Required columns / policy |
|---|---|---|
| `principalScoped` | class A — never portal-reachable (staff, junctions, TimeEntry, RateCard, WorkflowState, CredentialSecret, ProjectUpdateInternalSnapshot, …) | `portal_deny`; **no `visibility` column** (a class-A table carrying one is a posture failure — it invites a future "just gate it" shortcut) |
| `clientScoped` | class B, client-scoped without a project (Document w/o `projectId`, ClientAsset, CredentialItem, Contact-facing structural rows) | `client_id`, `visibility`, policy named `portal_gate` in the `(client_id, visibility)` form |
| `projectScoped` | class B with a `projectId` (WorkItem, WorkItemActivity, Comment, ProjectUpdate, Document with `projectId`, Milestone, ProjectVersion, Service, ProjectTimeSummary, `search_index` rows with `projectId`) — plus `Project`'s **own** row, which is structural: its gate is `client_id = app.client_id AND portal_enabled` with no `visibility` term (DATA_MODEL.md §2.3), the one exception the posture test allows | `client_id`, `visibility`, **`portal_enabled`**, policy named `portal_gate` in the three-term form (§7.2) |

`Notification` is registered as the one named **special** case (DATA_MODEL.md §2.3 "(special)" row): principalScoped *shape* (no `visibility` column) plus a per-receiver contact policy (`receiver_type = 'CONTACT' AND receiver_id = app.principal_id AND client_id = app.client_id`, read + `read_at`/`archived_at` UPDATE only — §7.2 census) instead of a bare `portal_deny`; the posture test asserts exactly that policy and no other exception.
...
2. First statement, single round trip, parameterized:
   ...
   **Amended 2026-08-16 (work-management plan, Phase 1b):** the same statement also sets **`app.principal_id`** — `set_config('app.principal_id', $4, true)` — the id of the member or contact (NULL/unset for `system` and `platform_admin`). It is transaction-local like the others (§4.2) and exists so RESTRICTIVE policies can pin a row to *this* principal, not just to its client: `Comment.author_contact_id = app.principal_id` on contact INSERT, `Notification.receiver_id = app.principal_id` on contact rows (§7.2). Fail-closed like `app.tenant_id`: unset ⇒ `NULL` ⇒ no match.
...
- **The one tenant-scoped table an auth flow must read is `contact`, and it gets a NAMED narrow admission rather than an exemption** *(added 2026-09-20, Phase 3 slice 1; migration `20260920210000_phase3_contact_identity`)*. The portal Better Auth instance maps its `user` model onto `Contact`, which is class B — so the sentence above collides with it: at sign-in there is no `app.tenant_id`, the isolation predicate is NULL, and the lookup returns nothing. The fix is the **`withUser()` shape** (§"my tenants", migration `20260808200000`), not a blanket `allow_runtime` and not `app_platform`: `contact_auth_lookup` admits the single row named by a transaction-local `app.auth_contact_email` **or** `app.auth_contact_id`, both set only by `src/db/portal-identity.ts` from a value the request already carried. Unset ⇒ `''`/NULL ⇒ no row, so deny-default holds. Three properties are load-bearing and each is pinned by `src/db/portal-identity.dbtest.ts`:
  - it is **SELECT-only**. There is no INSERT policy, which makes *invite-only* a property of the database rather than of `disableSignUp` alone; and no UPDATE policy, because a tenant-less write to `contact` cannot work anyway — the table feeds `search_index`, whose own RLS wants a tenant, so such a write dies on `42501`. Auth-path writes are therefore **brokered** through `withTenant(tenantId, {type:'system'})` in the tenancy the lookup just proved, like every other contact-caused write (AUTHZ.md §8);
  - the brokered write carries `app.auth_contact_id` into that transaction so the `contact_auth_path_immutable` trigger stays live: **tenancy, client, `portalProfile` and `portalStatus` cannot be changed by anything the auth plane caused.** They decide what a login can reach and belong to the tenant path under `client:manage_contacts`;
  - the RESTRICTIVE `portal_gate` on `contact` is untouched and still applies — it passes on the auth path only because `app.principal` is unset there, never because a contact is trusted.
...
### 6.4 RLS performance edges that shape the schema *(added 2026-08-16 — work-management plan)*

Blanket FORCE RLS has two planner consequences that are cheap to design around now and expensive to discover with tenant data in place. They are schema rules, not tuning notes:

- **Non-leakproof quals never become index quals.** Under RLS the planner only pushes *leakproof* operators below the policy filter; `@@` (tsvector), `%`/`ILIKE` (pg_trgm), `@>` on arrays are **not** leakproof, so a GIN/trgm index on a FORCE-RLS table is effectively never chosen — the plan slices on the btree `(tenant_id, …)` policy columns, then filters. **Rule:** no GIN/trgm indexes on tenant-owned tables in v1; full-text lives in a **narrow, trigger-fed `search_index`** table (`entityType, entityId, clientId?, projectId?, visibility, portal_enabled, title/subtitle/bodyText ≤ 100k, lang regconfig, search tsvector GENERATED … STORED`, per-row `lang` = `fortleva_sv` / `fortleva_en` from `Tenant.defaultLocale`) with only btree indexes `(tenant_id, updated_at)`, `(tenant_id, project_id, updated_at)`, `(tenant_id, client_id, visibility, updated_at)` — the tenant/project slice is small enough that a sequential `@@` over it is fine at target scale. The existing `document (tenant_id, tags)` GIN index is in this class and is not relied upon. Escape hatch (documented, not built): a `SECURITY DEFINER` search function that pre-slices by tenant under its own checks — only if a measured tenant needs it.
- **Recursive CTEs re-evaluate policies per scan.** A `WITH RECURSIVE` over `work_item` pays the policy predicate on every iteration; a project-wide progress query written that way degrades with hierarchy size and *also* re-runs the portal gate per level. **Rule:** rollups (progress %, Σ estimate, hours, budget burn) are **flat `GROUP BY` over denormalised columns** — `project_id`, `client_id`, `root_id` (top-level ancestor, maintained by trigger), `milestone_id`, `state_category`. Bounded recursion (depth ≤ 2 by CHECK) is allowed in exactly one place: the subtree pane of a single item. `ProjectTimeSummary` exists as a **physical class-B table** for the same family of reasons — a SQL view over class-A `time_entry` returns zero rows to a contact under FORCE RLS (DATA_MODEL.md).
- Corollaries already stated elsewhere but load-bearing here: InitPlan-wrapped `current_setting` (§6.2), `tenant_id`-leading indexes (§8.3), and no policy that joins or subselects another RLS table (§7.2 amendment).
...
Column placement rule (DATA_MODEL.md enumerates): **any row a portal query can render either carries `(clientId, visibility)` itself or is unreachable by portal queries.** Content tables (Document, ProjectVersion, Milestone, ~~IssueComment~~ Comment, WorkItem, ProjectUpdate, ProjectTimeSummary, CredentialItem, ClientAsset, `search_index` — *list amended 2026-08-16*) carry `visibility`; structural client rows (Contact, ~~Issue~~, ContinuityBox, Invoice, Contract) are gated by client match plus **status** where lifecycle applies (an Invoice is portal-visible only once issued/sent; a Contract only once sent — status gates live in queries *and* in the portal capability layer, AUTHZ.md). FileObject/FileVersion are never queried by contacts directly: portal downloads exist only as short-lived signed URLs issued *after* a data-layer check of the owning Document's visibility (SECURITY.md).
...
**Amended 2026-08-16 (work-management plan) — the project gate: `portal_enabled` denormalised onto every project-scoped class-B table.** `Project.portalEnabled Boolean @default(false)` is the per-project master switch. It must bind at the **data layer** — a projection-only gate means one `authorizePortal()` bug leaks a `CLIENT_VISIBLE` item of a not-yet-enabled project. Mechanism (decided): every **projectScoped** table (WorkItem, WorkItemActivity, Comment, ProjectUpdate, Document with `projectId`, Milestone, ProjectVersion, Service, ProjectTimeSummary, `search_index` rows with `projectId`) carries `portal_enabled boolean NOT NULL DEFAULT false`, stamped from the project on INSERT (BEFORE INSERT trigger) and kept in sync by **`AFTER UPDATE OF portal_enabled ON project`** which fans out `UPDATE <child> SET portal_enabled = NEW.portal_enabled WHERE tenant_id = NEW.tenant_id AND project_id = NEW.id` to every registered child **in the same transaction** (fan-out is bounded per project; the trigger body is generated from the registry so a new projectScoped table cannot be forgotten). The gate for these tables becomes:

```sql
CREATE POLICY portal_gate ON work_item
  AS RESTRICTIVE FOR ALL TO app_runtime
  USING (
    (SELECT current_setting('app.principal', true)) IS DISTINCT FROM 'contact'
    OR (
...
**Client-scoped tables without a project** (Document with `NULL projectId`, ClientAsset, CredentialItem) keep the two-term `(client_id, visibility)` form above — there is no project switch to honour. Class-A tables keep `portal_deny` and carry **no `visibility` column** at all (`TimeEntry`, `RateCard`, `WorkflowState`, `CredentialSecret`, `ProjectUpdateInternalSnapshot`, …); the portal reads their projections through class-B rows (`WorkItem.stateCategory`, `ProjectTimeSummary`, `CredentialItem`) — never the tables.

*Why not a subquery / join to `project` inside the RESTRICTIVE policy* (`AND EXISTS (SELECT 1 FROM project p WHERE p.id = project_id AND p.portal_enabled)`)? Rejected. Under FORCE RLS the subselect re-evaluates **`project`'s own policies per row** for the *contact* principal — so `project` would itself have to be `CLIENT_VISIBLE` to that contact, and any later change to `project`'s gate silently changes every child's gate. The two failure modes are exactly the worst-bug class: **fail-open** (a permissive path on `project` widens children) or **silent-empty** (the portal shows nothing and nobody notices until a client calls). A pure column comparison has neither; it costs one boolean per row and one trigger, and it is testable by grepping `pg_policies`. The rule generalises: **no policy on any tenant-owned table joins or subselects another RLS-governed table.**

*Registry and posture test* (`src/db/model-registry.ts`, `isolation.dbtest.ts` — §1.1 subclasses, §11): every `projectScoped` model must have columns `client_id`, `visibility`, `portal_enabled` **and** a policy named `portal_gate` in `pg_policies` whose `qual` contains `portal_enabled`; every `clientScoped` model: `client_id`, `visibility`, `portal_gate`; every `principalScoped` model: a policy named `portal_deny` and **no `visibility` column**. A model in none of the three subclasses fails the build (extends the existing "no `tenantId`" census). Behavioural test in the same suite: seed a portal-enabled project with `CLIENT_VISIBLE` rows in every projectScoped table, flip `portalEnabled=false` as a member, then read every one of those tables as the contact ⇒ **0 rows across all tables**; flip back ⇒ the rows return. Portal UI is Phase 3, but the columns, triggers, policies and this test land with the tables (Phase 2 / 2W) so the gate is exercised by data-layer tests long before a portal exists.

**The contact-writable set, enumerated exactly** (asserted in CI, §11 — a model entering this set is a deliberate, reviewed change):

**Amended 2026-08-16 (work-management plan).** ~~`Issue` and `IssueComment` rows~~ — **superseded**: `Issue` is absorbed into `WorkItem(kind=REQUEST)` and `IssueComment` into the polymorphic `Comment`. Request creation is *not* a contact-principal INSERT any more (see the brokered-write rule below); the census is now exactly:

| Table | What a contact may write | Note |
|---|---|---|
| ~~`Issue`~~ | ~~INSERT (own client), reporter-authored fields~~ | superseded 2026-08-16 — REQUEST creation is a brokered write into `WorkItem` |
| ~~`IssueComment`~~ | ~~INSERT (own client)~~ | superseded 2026-08-16 by `Comment` (next row) |
| `Comment` *(added 2026-08-16)* | **INSERT only**, on a subject the contact can already see | `WITH CHECK (visibility = 'CLIENT_VISIBLE' AND client_id = app.client_id AND author_contact_id = app.principal_id)` (the `portal_gate` on `comment` ANDs `portal_enabled` on top); no UPDATE/DELETE (edit/delete of a contact comment is a member act) |
...
| `Document` *(added 2026-08-16)* | UPDATE of the **approval columns only** (mirroring ProjectVersion: `approvalStatus`, `approvalDecidedAt`, `approvalByContactId`, `approvalNote`) on `kind = DELIVERABLE` | Phase 3 deliverable sign-off; same allowlist mechanism; still **no INSERT** — uploads stay brokered |
| `Notification` *(added 2026-08-16)* | UPDATE of `readAt` / `archivedAt` **only**, on rows where `receiver_type = 'CONTACT' AND receiver_id = app.principal_id AND client_id = app.client_id` | the row-pin uses `app.principal_id` (§4.1); the column allowlist is a column-level GRANT (`UPDATE (read_at, archived_at, snoozed_till)` — the third column is granted too, corrected 2026-09-20 by the census) plus a per-row policy — a contact can never mark another contact's row |
| `ContinuityOpenRequest` | INSERT + withdraw | `CONTACT_PRIMARY` profile only (AUTHZ.md §8, CONTINUITY_BOX.md §3.2) |

**ENFORCED IN THE SCHEMA SINCE 2026-09-20 — and until then the schema did not say this at all** *(Phase 3 slice 2; migrations `20260920230000` + `20260920233000`)*. The table above was a specification that the policies did not implement, measured rather than suspected: `tenant_isolation` is PERMISSIVE **FOR ALL** on every class-B table and `portal_gate`'s WITH CHECK only pinned the row to the contact's own client, so a contact principal could INSERT, UPDATE and DELETE rows of `client`, `contact`, `project`, `project_version`, `milestone`, `service`, `document`, `project_time_summary` and `time_report`, and could DELETE any CLIENT_VISIBLE `work_item` / `work_item_activity` (those two denied INSERT and UPDATE through `portal_gate`'s WITH CHECK, but a DELETE is decided by USING, and USING is the READ gate). The sharpest was `contact`: a **self-signup and a self-promotion to `CONTACT_PRIMARY`** at the data layer, against the invariant slice 1 had closed three separate ways on the AUTH path — none of which is in force inside an ordinary contact transaction. None of it was reachable, because no application code had ever handed a contact-principal transaction to a write; that is exactly the point, and why it was closed before the slices that start handing contacts real surfaces.

Three named RESTRICTIVE policies per class-B table now carry it — `portal_no_insert` / `portal_no_update` / `portal_no_delete` — named rather than folded into `portal_gate`'s qual so that opening one is a visible `DROP POLICY` in a migration and not a two-character edit inside a boolean. **`portal_no_update` is a `WITH CHECK` deny with no `USING`, and that is load-bearing:** Postgres applies the UPDATE policies to a row LOCK, so a RESTRICTIVE `FOR UPDATE … USING` deny also makes `SELECT … FOR SHARE` return nothing — which broke `comment_denorm_guard` and with it the one permitted contact write, reported as `COMMENT_SUBJECT_GONE`. Only the census test's POSITIVE CONTROL could see it; every structural assertion and every denial test passed harder than before. `portal_no_delete` stays a USING deny (DELETE has no WITH CHECK, and a row lock consults UPDATE policies only).

**The census is now a computed property of the database, at column granularity** (`src/portal/census.dbtest.ts`): over every table in `public`, for INSERT/UPDATE/DELETE, combining `pg_policies` with `has_table_privilege` **and** `has_column_privilege` — the latter because `notification`'s entry is a column GRANT, which `has_table_privilege` reports as absent. Measured result, pinned: `audit_event` INSERT (portal actions are audited; append-only, and `audit_portal_select_deny` means a contact cannot read back what it wrote) · `comment` INSERT · `notification` UPDATE of `read_at`, `archived_at` **and `snoozed_till`** (the third column is granted and the documents above name only two — a drift in the docs, not in the schema) · `search_index` INSERT/UPDATE **pinned to `entity_type = 'COMMENT'`**, which is not a principal-sense entry at all but the feed trigger firing underneath the permitted comment INSERT. *(Migration `20260921000000`, after both fresh reviews: the first cut left `search_index` DELETE open and bound no `entity_type`, so a contact could have emptied or polluted the tenant's own staff search; and `audit_event` INSERT was unbound, so a contact could append a permanent audit row attributed to a MEMBER. Both closed; `comment` also gave up the last `USING`-shaped deny.)* Adding a write means dropping the named policy in a migration, extending the census constant, and shipping the behavioural test **in the same commit**.

**The brokered-write rule, generalised** *(added 2026-08-16)*. **Every other contact-caused write** — creating a REQUEST (`WorkItem kind=REQUEST, source=PORTAL`, forced `CLIENT_VISIBLE`, TRIAGE-category state), completing an own-assigned task (`stateCategory` → DONE on `assigneeContactId = self` only), submitting a credential (3V), consuming a share link (3V), file uploads (below) — runs as: `authorizePortal()` under the contact principal (capability + row visible to that contact) **then** re-enter **`withTenant(tenantId, {type:'system'})`** to perform the write with every forced column stamped server-side. These functions live in **`src/modules/<key>/portal-writes.ts`** and nowhere else — *`portal.ts` until 2026-09-20, when the founder split the writes into their own file so that "this code runs as system" is a property of a filename; the forbidden-columns grep follows both names, and `src/portal/brokered-writes.test.ts` (2026-09-21, with the first such write) pins that every `withTenant` in one takes an inline `{type:'system'}` and that `authorizePortal` is called before it opens.* Two hard corollaries: (a) **`withPlatform` is never called from a portal or tenant route** — an ESLint import-boundary rule makes `withPlatform`/`getPlatformClient` importable only from `(platform)` route code and `jobs/` (§12); (b) **portal *reads* never run under a system principal** — every portal SELECT executes under the RLS-scoped contact principal so the gate, not the projection, is what limits the rows. "View as Contact" reuses the identical read functions (import-graph asserted). Test consequence: the census test must show that a contact-principal INSERT into `work_item` (any kind) is *rejected*, while the brokered path produces exactly one REQUEST row with the forced values. *(Both halves shipped 2026-09-21 with the intake: `src/modules/work/portal-writes.dbtest.ts` drives the rejection and the forced row side by side, and the census itself is UNCHANGED — no policy was dropped for this write.)*

**Contact file uploads do not widen this set — they are brokered** (decided; stated identically in DATA_MODEL.md §2.3 and SECURITY.md §5). A request attachment (~~issue~~ — on a `WorkItem(kind=REQUEST)`, Comment or ProjectUpdate) is not a contact-principal INSERT into the file layer: the server action runs `authorizePortal()` first, then re-enters `withTenant()` as the **`system` principal** to create the `Document` / `FileVersion` / `FileObject` rows (forced `clientId` = the contact's client, forced `CLIENT_VISIBLE` on the `Document`). `FileObject`/`FileVersion` therefore keep `portal_deny` with **no INSERT exception**, `Document` stays writable only by members and the system principal, and the `createdByContactId` / `uploadedByContactId` columns in DATA_MODEL.md are **attribution only** — who caused the upload, never who performed the write. The alternative (a narrow INSERT-only portal policy on the file layer) was rejected: it puts a write grant on the exact tables whose accidental exposure §1 calls the worst bug this product can have, to save one server-side principal switch.
...
- **Contact-writable census:** enumerate every model via DMMF and attempt an INSERT/UPDATE as a `contact` principal on each; the set that succeeds must equal **exactly** the §7.2 table (~~Issue, IssueComment, ContinuityOpenRequest, ProjectVersion approval columns~~ — **amended 2026-08-16:** `Comment` INSERT, `ProjectVersion` approval columns, `Document` approval columns, `Notification.readAt/archivedAt` on own contact rows, `ContinuityOpenRequest`). Any other success — notably `Document` INSERT, `FileVersion`, `FileObject`, `WorkItem` (any kind), `TimeEntry`, `CredentialSecret` — fails the build. The census file is updated **in the same commit** as any feature that touches it. The brokered paths are tested separately: `authorizePortal` → `system`-principal `withTenant` produces exactly one `Document` (CLIENT_VISIBLE, contact's client) + one `FileVersion` + one `FileObject` with the contact recorded in the attribution columns; likewise exactly one `WorkItem(kind=REQUEST, source=PORTAL, CLIENT_VISIBLE, TRIAGE)` for a request, and a state change only on an own-assigned item.
- **Posture assertions:** every tenant-owned table has `relrowsecurity AND relforcerowsecurity`; `app_runtime` has `rolbypassrls = false`; UPDATE/DELETE on `audit_event` denied. **Amended 2026-08-16 — registry subclasses (§1.1, §7.2):** every model is in exactly one of `principalScoped` / `clientScoped` / `projectScoped`; `projectScoped` ⇒ columns `client_id`, `visibility`, `portal_enabled` + policy `portal_gate` whose qual references `portal_enabled`; `clientScoped` ⇒ `client_id`, `visibility`, `portal_gate`; `principalScoped` ⇒ `portal_deny` and **no `visibility` column**; the `project` fan-out trigger lists every `projectScoped` table (generated from the registry, diffed in CI). Behavioural: `portalEnabled=false` ⇒ **0 rows for a contact across every projectScoped table**.

**Cross-cutting tripwires** *(added 2026-08-16 — work-management plan; each ships in the same commit as the feature it guards, and every phase's definition of done includes them)*:

- **Portal forbidden-columns grep** on `src/modules/*/portal.ts` *(and `portal-writes.ts`, added 2026-09-20)*: no select/return of rates, cost, `internalNotes`, `repoUrl`, `hostingNotes`, non-billable seconds, per-member breakdowns, state *names*, labels, links, `assigneeMemberId`, `Contact.invitedById`, INTERNAL activity rows, ciphertext columns; the allow-list per projection is explicit and the grep fails on anything outside it. **Since 2026-09-21 the "explicit allow-list" half is checked STRUCTURALLY and not merely promised** (`src/authz/portal-projections.test.ts` parses the AST): a read with no `select`, an `include`, an `omit`, raw SQL, a `select` that is not an inline literal, or a relation selected as `true` — which returns every column of the related row while naming no forbidden one — each fails the build. Keys are resolved against `prisma/schema.prisma`, so a relation must carry a nested allow-list of its own. A name list cannot know about an internal column added next year; this can.
- **"No INTERNAL fact to a Contact" fixtures:** INTERNAL child of a CLIENT_VISIBLE parent, INTERNAL comment on a CLIENT_VISIBLE item, unpublished `ProjectUpdate`, hours widget when `hoursSharingMode=NONE`, per-member breakdown, state names, labels — each read as a contact ⇒ absent; "View as Contact" returns byte-identical JSON to a real contact session.
- **Search lexeme probe:** a word that occurs only in an INTERNAL body (or in an enabled project's INTERNAL row, or in a disabled project's CLIENT_VISIBLE row) never matches under a contact principal; the same word matches for a scoped member.
- **Kind-catalog audience test:** every notification kind with `audience` including CONTACT is `clientVisibleOnly`, and a fixture asserts an INTERNAL change never produces a CONTACT-receiver `Notification`/outbox row.
- **`withPlatform` import-boundary test:** `withPlatform`/`getPlatformClient` are unreachable (import graph) from `(tenant)`/`(portal)` route groups and from `src/modules/*/portal.ts`; only `(platform)` and `jobs/` may import them (§7.2 brokered-write rule, §12).
- **Contact-writable census** (above) and **registry posture test** (above) are part of the same tripwire set.

**Feature-critical property tests** *(added 2026-08-16)*, run alongside the suite (against the service container per push, and against real Neon when `neon-smoke.yml` is dispatched — amended 2026-09-01, see Infrastructure below): **rank uniqueness** — 50 concurrent moves in one project never violate `UNIQUE (tenant_id, project_id, rank)` and never lose an item; **one running timer** — two concurrent `timer.start` for the same member ⇒ exactly one row with `stopped_at IS NULL` (partial unique index), the other auto-stopped in the same tx; **`ProjectTimeSummary == SUM(time_entry)`** per (project, month) after any random sequence of entry create/edit/delete/reprice, and the summary table has no member column by construction; plus rate-snapshot stability, lock-trigger bypass being transaction-local, AAD binding, and share-link view-once atomicity (owned by SECURITY.md / DATA_MODEL.md but run here).
- **Infrastructure** *(amended 2026-09-01 — founder-approved; the previous text specified an **ephemeral Neon branch** per run and forbade a local database outright)*: each CI run brings its **own `postgres:18` service container** on the runner, migrates it **from empty**, seeds it, and runs the suite **as the real `app_runtime` role**. The original prohibition's *reason* stands and is now enforced rather than assumed: a local role is typically owner/superuser, so RLS false-passes ([Neon's own guidance](https://neon.com/guides/test-rls-on-neon-branches)) — therefore `isolation.dbtest.ts` asserts `current_user` is `app_runtime` (and `app_platform` on the platform seam), and a run that silently fell back to the container's superuser now **fails** instead of passing green. The roles themselves are the same ones production has, created by the `security_foundations` migration, not by a console. What a container cannot speak for is the **pooler** — §4.2's "no design here may rely on session state, ever" — so that is bought back deliberately by `.github/workflows/neon-smoke.yml`: a **manual** run of six chosen files against the real Neon, before a deploy, before pushing any migration that carries DML, or after anything touching pooling, transactions or the db seam. **What is bought back is pooler SEMANTICS, not production's numbers:** the smoke job crosses the same ~100 ms link the container exists to remove, so it still sets `DB_TX_TIMEOUT_MS` and therefore runs with `maxWait` at 8 s rather than 2 s. **The real 2 s connection ceiling — the `maxWait` exhaustion class — is exercised by NOTHING in either pipeline.** Only a co-located runner would close that, and it is recorded as an open gap rather than papered over. The branch design was costed and rejected on 2026-09-01, not abandoned: GitHub bills the **sum** of job-minutes, so a private branch halves wall clock and saves no billable minutes, while the container removes the US↔EU latency that is the actual cost (PLAN §0). The container also removes every real-database credential from the automatic pipeline. This all still matches [AWS SaaS Lens REL_3](https://wa.aws.amazon.com/saas.question.REL_3.en.html) on continuously testing tenant boundaries.
...
- **System jobs** (retention cron, Stripe webhook → entitlements refresh) run as the `system` principal through the same `withPlatform` seam; ~~each write audited~~ — **amended 2026-08-16 (work-management plan):** with `*/2` outbox drains, `*/15` timer sweeps, hourly budget checks and nightly summary self-heal, one `platform.system_job` row *per invocation* is ~800 valueless audit rows a day that inflate the retention set. The rule is now: a system job audits **one summary `job.run` event per run** (catalogued action; metadata = job name, tenants touched, counts processed/failed, duration, outcome), written at the end of the run under `withPlatform`; **per-tenant effects are audited inside the re-entered `withTenant(tenantId, {type:'system'})`** with their own catalogued actions (`timer.auto_stopped`, `budget.alert_sent`, `expiration.reminder_sent`, `search.index_rebuilt`, …) so they land in *that tenant's* log exactly as before. `withPlatform` gains an option **`skipInvocationAudit`** that suppresses the per-invocation row; it is **reserved for `type:'system'` actors** — passing it with a `platform_admin` actor throws, so human platform access stays one-row-per-invocation with a mandatory reason. Jobs that touch a tenant's rows do so through `withTenant`, never through raw `app_platform` writes; the platform connection is used only to enumerate tenants / drain the outbox with `FOR UPDATE SKIP LOCKED`.
...
| Item | Scope |
|---|---|
| Shared schema + `tenantId`; `withTenant` UoW; where-injection extension; RLS (all tenant-owned tables, portal RESTRICTIVE gates); composite FKs + tenant-scoped uniques + tenantId-leading indexes; role split + separate URLs; CI isolation suite on a per-run Postgres container as `app_runtime` (+ manual real-Neon smoke); `Tenant.databaseUrl`/`cell` seam; `withPlatform` audited bypass | **v1** (Phase 1 foundations — §11 of the brief: never retrofit tenancy) |
| *(added 2026-08-16 — work-management plan)* `app.principal_id` GUC; registry subclasses `principalScoped` / `clientScoped` / `projectScoped` + posture test; denormalised `portal_enabled` + fan-out trigger + three-term `portal_gate` on every projectScoped table; revised contact-writable census + brokered-write rule; `job.run` summary audit + `skipInvocationAudit`; tripwire and property tests (§11); RLS performance rules (§6.4) | **v1** — Phase 1b (GUC, subclasses, `requireRecentMfa`), Phase 2/2W (columns, triggers, policies, tests as tables land), Phase 3 (portal UI over an already-tested gate) |
