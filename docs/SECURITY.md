...
- **Secrets:** Vercel env vars only; nothing in the repo — *one carve-out, 2026-09-01: throwaway CI-only values for the ephemeral fixtures database (`BETTER_AUTH_SECRET`, `FIELD_ENCRYPTION_KEY`) are committed in `.github/workflows/*.yml` and named there; production values never appear outside the deployment environment, and no automatic CI job holds a credential to a real database*; keyring offline copy per §6; R2 token scoped to the one bucket with minimum verbs; runtime vs migrate DB roles separated (§7).
