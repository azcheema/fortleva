...
| Resend or Postmark (ESP — final pick in `OPEN_QUESTIONS.md`) | Transactional email: invites, notifications, continuity alerts | EU region option required at selection; email inherently transits ESP infrastructure | DPA + SCCs. Hard rule: **no key material, credentials, or box contents in email, ever.** |
