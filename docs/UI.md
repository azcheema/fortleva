...
|---|---|---|
| Home | `/home` | Rule 8. *2026-08-21 (landed):* the **"Your time" strip** — two `MetricTile`s (this week / today, the member's OWN finished hours; a running timer's elapsed is added live from its server start, counted into each only from its own midnight) and the **timer slot** (a Running tile ticking, or a ghost "Start a timer" link to the quick start); shown only to a member who can track time; the rest of rule 8 (assigned / waiting / triage / inbox) arrives with the 2W UX finish. All live clocks outside the pill share `useServerNow` (`src/components/shell/use-server-now.ts`). |
