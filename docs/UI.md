...
**The shipped `<PropertyPicker>` contract (slice 5 on 2026-09-12, slice 6 on 2026-09-13 — `src/components/semantic/property-picker.tsx`).** `S P E D` use it; `V M L` reuse it as it stands now, slice 6's two optional seams included. Settled, do not re-litigate:
...
- The rail's islands share ONE commit path, `usePanelCommit` (`item-panel/use-panel-commit.ts`): a transition, `useOptimistic` over required server props, a no-op guard, adopt-and-refresh on both `changed` branches and on failure, and a sequence token so a commit superseded by a newer pick adopts, announces and refreshes nothing when its answer lands (a superseded failure still toasts; and if the newest pick then fails after a superseded one answered, that answer's canonical value is announced — changed or not, it is the newest stored value, what the item ends on).
...
**The backlog table's S/P/E/D cells stay `InlineEdit`s** (a default this session took on 2026-09-13; it awaits the founder's ratification — PLAN §0). They already write through the same four `ActionResult` actions as the panel, but the picker's trigger is `h-8` where a table row's control is `h-7` (`density="table"`), and `e2e/work.spec.ts` pins the native controls — converting them waits for a table-density trigger, in a backlog slice of its own.
...
| item | `S` · `P` · `E` · `D` | State · Priority · Estimate · Due date pickers (§5.2) — **shipped** (`S` in slice 5, 2026-09-12; `P E D` in slice 6, 2026-09-13); `A L V M X` follow in their slices |
