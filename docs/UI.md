...
| item | `S` · `A` · `P` · `E` · `D` · `V` | State · Assignee · Priority · Estimate · Due date · Visibility pickers (§5.2) — **shipped** (`S` in slice 5, 2026-09-12; `P E D` in slice 6 and `A V` in slice 7, both 2026-09-13); `L M X` follow in their slices. `G A` (account) coexists with the rail's bare `A` exactly as `G P` and `G S` do — an armed `G` consumes the next key first |
