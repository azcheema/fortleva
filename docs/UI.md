...
**The shipped `<PropertyPicker>` contract (slice 5 on 2026-09-12, slices 6 and 7 on 2026-09-13 — `src/components/semantic/property-picker.tsx`).** `S A P E D V` use it (that is also their rail order, and so the `?` overlay's); `M L` reuse it as it stands now, slice 6's two optional seams included — slice 7 needed neither seam. What slice 7 adds is two islands under the rules below: `A` lists the tenant's ACTIVE members in the list surfaces' order with the current one checked in place, the "Unassigned"/"Unassign" pair following the empty-property rule, and no contacts (Phase 3 owns `assigneeContactId` and the "make it client-visible?" warning); `V`'s trigger rest state IS the `<VisibilityBadge>`, so the editable chip and the read-only chip are one silhouette, its refusal is the database's own (`HAS_VISIBLE_CHILDREN`, naming what to make private first), and it is the one island that is never optimistic. Settled, do not re-litigate:
