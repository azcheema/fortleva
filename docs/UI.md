...
**No visual change ships on a screenshot of one page.** Every change to a token, a component or a screen is verified with the browser visual sweep — `pnpm test:e2e` (`e2e/visual.spec.ts`, `RUNBOOK.md` §7): 46 stops × light/dark × 1440×900 and 390×844 = 184 shots, each stop audited in the page for exactly one `h1`, text that composites to its own backdrop, untranslated message keys, horizontal overflow at phone width, broken images, and console/network errors. The shots land in `.design-shots/` (git-ignored) and are reviewed in both themes before the commit. A contrast table that passes proves the palette; only the sweep proves the pages.

**The craft audit** (`e2e/audit.ts`, run at all 184 stop/theme/device combinations — 46 stops × light/dark × desktop/phone) is the half of this document a screenshot cannot hold open. Each of these must be empty on every stop, or the run fails:
...
**The cue must survive the last row.** `TableBody` carries `[&_tr:last-child]:border-b-0`, **not** `border-0`: `border-0` zeroes all four widths, including the `border-left` the cue paints — which meant the last client-visible row of every table in the product silently lost its safety marking while the legend below it asserted it in words. `/settings/design`'s visibility demo puts a client-visible row **last** on purpose, so the showcase proves this rather than hiding it, and `e2e/visibility.spec.ts` asserts the computed `border-left-width` on **every** row of a real table — the last one included — plus that a client-visible row's `border-left-color` differs from an internal row's and is not alpha-0. *(Added 2026-09-02 with 2W-F slice 2. Until then this sentence described a guard that did not exist: the suite contained no `border` assertion at all. Nothing else can catch the trap — the craft audit's `rowPitch` measures row HEIGHT, which a missing left border does not change, so all 184 stops are blind to it.)*
