...
- **A CODE BUMP HAS NO MIGRATION, and the half an agent may run is not the half a RELEASE owes** *(2026-09-22, TEMPLATE_VERSION 6)*. `prisma/seed.ts` does two things: it upserts the global `Permission` catalog — platform-level, idempotent, additive — and it then runs B3 propagation, which writes `RolePermission` rows into **every existing tenant's** roles. Only the first is safe for an automated agent (AGENTS.md forbids writing roles outside a throwaway tenant, and forbids the `naxdor` tenant outright), and only the first is needed for tests: `src/members/templates.ts` grants a role its codes BY CODE, so a tenant provisioned after a catalogue addition silently misses the new one until the `Permission` row exists. `scripts/seed-catalog.ts` is that half, and says so. **The release still owes the full `prisma/seed.ts`** — without the propagation, an existing tenant's owners and managers do not hold a code the code path requires.
- `authorize()` **fails closed on unknown codes**: deny + alert (it's a config error, not a user error).
- **A CODE MAY EXIST BEFORE ITS ENFORCEMENT SITE, and thirty-six do** *(audited 2026-09-20; `work_item:triage` left the list on 2026-09-22 when Phase 3 slice 6b gave it `src/modules/work/triage.ts` — the oldest name on it)*. That is a consequence of the immutability rule two bullets up, not a gap: codes are forever, so declaring a module's set when the module is DESIGNED — rather than when it ships — is what stops a later slice minting a second spelling for the same capability. Four whole modules are waiting on features that do not exist (contracts, invoicing, reports, continuity_box), plus `billing:*` in core, the five `issue:*` are deprecated and will never gain one, and the rest name capabilities whose WRITE PATH is absent: `audit:view` (there is no audit VIEWER — no screen, no route; the log is not write-only, since the tenant's own data export dumps it through `EXPORT_MODELS`, but that path is gated by `tenant:export` because the export is all-or-nothing by design), `workflow:manage` (the app creates a project's default states lazily and reads them; it has never updated or deleted one), `project_update:*` and `project_template:manage` (no service, no route), and `time:manage_locks` (the lock is REAL at the database — a locked entry refuses every edit and the time week renders a `Locked` badge — but nothing in the app writes `TimeEntry.lockedAt`; the writer is invoicing's, Phase 4, so the UI is ready for a state that cannot yet occur).
- **ONE CODE WAS NOT AHEAD OF ITS FEATURE — IT WAS SIMPLY UNENFORCED** *(found and fixed 2026-09-20)*. `member:view` had exactly one occurrence in the product: the `permission` on its **nav entry**, which hides a rail link. `/members` itself ran on `requireTenantContext()` alone and handed the full roster and every pending invite's email to any member who typed the URL. All four seeded templates carry `member:view`, so only a hand-built custom role could reach it, and only inside its own tenant — RLS never wavered — but the permission existed, the rail honoured it, and the page did not. It is guarded now, shaped like `/settings/roles`: the read is refused and the page renders the forbidden state rather than 404ing. **A nav entry is a visibility hint, never a gate**, and the test below now excludes `nav.ts` from its scan so a link can never again make a code look locked.
- **THE PAIR IS PINNED BY A TEST**, `src/authz/enforcement.test.ts`, because the audit that produced the list above was a script run once by hand and PLAN §0 had been carrying "four permission codes with no enforcement site" — a count scoped to the §§2W/2T audit and then quoted as a total. The test asserts an EQUALITY, not a subset, so it fails in both directions: a new code with no site and no declaration fails, and so does a DECLARED code that gains one — which is the half that earns it, since the day invoicing ships is the day `invoice:issue` must leave the list and a reviewer must be asked whether its new guard is in the right place. Removing a code from `DECLARED_AHEAD` belongs in the same commit that guards it.
...
| `work_item:triage` | work | Answer a `REQUEST` in `TRIAGE` — **enforced 2026-09-22**, `src/modules/work/triage.ts`. Its own code and not a flavour of `work_item:edit`: accepting work commits the agency to it. Alone it grants **Accept and Snooze**; Decline and Duplicate need the row below as well. An `edit`-only member can still drag a request into "To do" (that is accepting it) but cannot end one | C M E | scoped | 2W |
| `work_item:triage_decline` | work | **Decline a client's `REQUEST`, or mark it a duplicate** — both publish the agency's reply to the client's portal verbatim. **C M, not C M E** *(founder decision, 2026-09-22)*: an employee may take a request on or park it, but ending one speaks to a client in the agency's name, which is a delivery lead's call in every other row of §3.2's portal column. This product has exactly one other code that puts member-written prose in front of a client — `project_update:publish` — and it is C M too. Duplicate is covered because from the client's side it IS a decline (`portal.ts` renders both as an answered request with the reason — "Declined", or "Cancelled" when the agency had accepted it first, C31), so a permission guarding one verb would guard nothing. **Supplements `work_item:triage`, never replaces it** — both are required, the shape `work_item:approve` takes over `work_item:edit` | C M | scoped | 3 |
...
| `project_update:view` | work | View `ProjectUpdate`s incl. drafts and the internal snapshot — *amended 2026-09-25 (Phase 3 slice 67, the first enforcement site): the snapshot's PARTS are each gated on read by the code that gates their live source (`time:view_team` for per-member hours, `budget:view` + `rate:view_bill` for the budget burn, `rate:view_cost` ✦ for cost and margin), so a frozen post never widens what a member could read live* | C M A E | scoped | 2W (UI Phase 3) |
| `project_update:create` | work | Draft and edit unpublished updates | C M E | scoped | 2W (UI Phase 3) |
| `project_update:publish` | work | Publish (freezes `seq` + snapshots), archive — *and, since 2026-09-25: retract within the fifteen-minute window, and add the one note that may change after publish (`editNote`); each changes what a client reads, which is the judgement this row reserves for a delivery lead* | C M | scoped | 2W (UI Phase 3) |
...
| `project:manage_portal` | portal | `Project.portalEnabled`, `hoursSharingMode`, task-list/kanban toggles, "View as client" — audited *(the "audited" binds the two WRITES, which record `project.portal_enabled` / `portal_disabled` / `hours_sharing_changed`, and the explicit **View-as-Contact** mode, which records `project.viewed_as_contact` — **shipped 2026-09-21 (slice 5)**: one row per ENTRY, targeting the project the member came from, metadata `{contactId, clientId, portalProfile}` and never the contact's name. Entry is a POST that writes `Session.viewAsContactId`, so being inside the mode without a row is not a reachable state; a `<Link>` would have written one per hover in production. The Portal tab's "what the client sees" panel records NOTHING: it is a page RENDER, re-run on prefetch and revalidation, over CLIENT_VISIBLE rows of a project the member already holds `project:view` and scope on. Founder disposition wanted on that half — PLAN §0.)* | C M | scoped | 3 (TV5, **landed 2026-09-21**) |
...
**`TEMPLATE_VERSION` 2 semantics *(added 2026-08-16 — work-management plan; the "currently" corrected 2026-09-21, when it was still saying 1 three versions later)*.** Templates are versioned as one integer (`src/authz/catalog.ts` `TEMPLATE_VERSION`, **currently 5** — TV2 2W, TV3 2T, TV4 2W-R, TV5 Phase 3); a bump is a platform migration of the four system roles under the B3 rule (§3.5, decided): **additions propagate, removals never do**. TV2 ships with Phase 2W and is defined as:
...
- **`scopeWhere(actor)`** — for a project-scoped table (anything with a `projectId` column — projects, milestones, versions, services, work items, comments, updates, time entries, budgets, project-attached documents and credentials): `{ OR: [ { clientId: { in: directClientIds } }, { projectId: { in: projectIds } } ] }` (`{}` under `all`) — **the lifted client ids are deliberately absent**, which is precisely what keeps P2 invisible to a P1-only member; for a client-scoped table without `projectId` (client-level documents, assets, credentials): `{ clientId: { in: directClientIds } }`; for `Client`: `{ id: { in: directClientIds ∪ liftedClientIds } }`. Composed *inside* the tenant-scoped client for every list `where`. **Corrected 2026-09-06: there is NO lint rule enforcing this, and this sentence used to claim there was.** `eslint.config.mjs` contains no `scopeWhere` selector at all — its only `no-restricted-syntax` block is the i18n JSX one — and the rule as previously described is not cleanly expressible: banning a hand-written `clientId: { in: … }` would false-positive on `src/clients/service.ts:129`, which narrows to already-loaded ids *alongside* a real `...projectScope`, and ESLint cannot tell that apart from a substitute for it. **What actually guards this is dbtests, not lint**: `src/authz/scope.dbtest.ts` ("scopeWhere composed into real list queries" — E1 sees P1's rows and not P2's, E2 sees nothing, owner sees all) and the cross-scope oracle tests in `src/modules/work/bulk.dbtest.ts`, which pin that an out-of-scope id must answer identically to a nonexistent one. A new list query is therefore covered only if someone writes that test for it — say so when reviewing one.
...
> **Build status, 2026-09-20 (Phase 3 slice 2).** `authorizePortal()` now exists — `src/portal/`: the frozen capability table and the two profile bundles (`capabilities.ts`), the pure verdicts (`policy.ts`), the module-gate reader (`module-gates.ts`), the seam itself plus `withPortalRead()` (`authorize.ts`) and `requirePortalContext()` (`context.ts`). No UI, no projection, no route. Still true: no contact can hold a session, because nothing in application code writes `portalStatus` away from `NO_ACCESS`.
>
> **Three amendments this slice makes to what is written below, each because the code met something the prose had not.**
>
> 1. **The module gates cannot be evaluated under the contact principal, and reading them there would have failed OPEN on all three.** `feature_flag`, `tenant_preference` and `tenant` all carry `portal_deny`, so a contact-principal read of each returns zero rows — and zero rows means *enabled* to every reader in the product (`flagEnabled` treats a missing flag as on, `parseEntitlements(undefined)` returns the everything-on default, `preferenceEnabled` treats a missing row as enabled). Gates 1–3 would have passed for every tenant, including one that had switched the portal off, and no test inside a contact transaction could have seen it. They are therefore resolved ONCE per request by `resolvePortalModuleGates(tenantId)` under a **system** principal and carried on the principal as data. This does not bend "portal reads never run under a system principal" (TENANCY.md §7.2): that rule is about *projections*, and the value here is a fixed-shape map from module name to one of four enum states — no row, no id, no string from the database.
> 2. **The pipeline's order is §8's, not §5's, and that is deliberate.** Gates are AND-ed so the outcome is identical either way; the order fixes only which reason comes back. Evaluating the module gates LAST means a contact who is not allowed anyway never learns whether their agency's plan includes invoicing.
> 3. **"was invited" is enforced on the ROW.** §8's gloss makes invite-only a property of the codebase ("no self-signup path exists in code"); `authorizePortal()` additionally requires `Contact.invitedAt` to be stamped, so an activation path that forgets to record the invitation is refused rather than trusted. Anything that activates a contact must set it.
>
> **The continuity exemption is CONDITIONAL, and its condition is enforced** (`sealedBoxExempt` in `capabilities.ts`, `continuityBoxSealed` in `policy.ts`). "Once a box is SEALED" is a precondition, not scene-setting: the first cut applied the exemption unconditionally, so a tenant that had never bought `continuity_box` passed gate 2 anyway and the commercial gate on continuity was not enforced for contacts at any time (both fresh reviews caught it). The exemption now applies only when a caller proves the box is sealed — default **false**, so Phase 8 cannot forget it. Which gates, for which module: gate 2 is waived for both `portal` and `continuity_box`, because a lapsed plan can drop the portal entitlement and seal the box with it, which §5 forbids; **gate 3 is waived only for `continuity_box`**, as §5 words it, because gate 3 is a `TenantPreference` an operator SET and a tenant that deliberately switches the portal off has said something the product must obey. Gate 1 and the profile gate always apply. Phase 8 owns the lifecycle and confirms this against CONTINUITY_BOX.md.
>
> **Denial reasons are internal — CLOSED 2026-09-21 by slice 3, for every AUTHORIZATION denial; admission is deliberately not covered.** `authorizePortal()` throws `AuthzError` with a precise reason for the audit row, the server log and the test matrix. The portal's HTTP surface renders every one of them identically, and it does so in ONE place rather than in every page: `portalReadOrNull()` (`src/portal/render.ts`) turns every `AuthzError` into `null`, and a portal page renders `null` exactly as it renders a legitimately empty answer. A contact whose agency's plan lacks the module, a contact whose agency switched the portal off, a contact whose profile does not hold the capability and a contact whose agency has simply shared nothing all get the same page, byte for byte. Three properties are load-bearing and each is asserted: it swallows **every** reason (`render.test.ts` walks the whole `DenialReason` union, so a reason added later cannot get its own branch); it swallows **only** `AuthzError`, because a page that says "nothing shared yet" when the database is down has told the client something false about their agency; and it **redirects nowhere**.
>
> **THE EXCEPTION, stated because the first draft of this paragraph claimed there was none and both fresh reviews of slice 3 caught it independently.** A SUSPENDED, REVOKED or unverified contact does NOT reach a projection: `requirePortalContact()` (`src/auth/session.ts`) redirects to `/portal/login` on any `portalGateDecision` verdict but `"ok"`, so `authorizePortal`'s own `FORBIDDEN "contact is not ACTIVE"` is unreachable from a page and is exercised only by a hand-built principal in a dbtest. That behaviour is correct and stays — a contact whose access was withdrawn must lose the session rather than keep a usable one in front of a blank page — and the disclosure runs the other way from the one this rule exists to stop: admission tells the reader a fact about **themselves**, which they need in order to ask for their access back and which signing in would tell them anyway, whereas `portalReadOrNull` hides facts about the **agency** (its plan, its settings, its other clients). The rule is therefore: *every authorization denial after admission renders identically; admission itself signs the contact out.*
>
> ~~and no application code creates a `ContactAccount`~~ — **that sentence was FALSE and both reviews of the slice caught it.** *Our* code does not, but Better Auth's `/reset-password` creates a credential when none exists, and merely configuring `sendResetPassword` mounts the unauthenticated `/request-password-reset` that issues the token. A contact a member had merely recorded could therefore have set themselves a portal password, bypassing the invitation entirely and lying dormant until somebody activated them. Closed at the database — `contact_account_requires_invite` (migration `20260920210000`) refuses a credential for any contact that is not INVITED or ACTIVE — and `sendResetPassword` declines to mail a non-ACTIVE contact, without changing the endpoint's deliberately constant response. Both are measured, including end-to-end through the real endpoints. *(Since the reset screens, 2026-09-24, the endpoint path stops one step earlier — `portalAuth`'s redemption guard burns a non-ACTIVE contact's token before the library can reach the trigger — so the trigger is pinned directly in `src/db/portal-identity.dbtest.ts` and the guard in `src/auth/portal.dbtest.ts`.)*
>
...
>
> **And "contact is active" is also checked, since the reset screens (2026-09-24), where a password-reset link is REDEEMED.** `/reset-password` updates an existing credential without asking whose, so `portalAuth`'s `hooks.before` burns the token of any contact who is not ACTIVE and lets the library refuse it with its own `INVALID_TOKEN` — rather than trusting every writer of `portal_status` to have purged the links, which the first purge did not (SECURITY.md §3.4). The new-password page asks the same question before it draws a form (`portalResetHolder`), so a paused contact meets the dead-link state, never two password boxes the endpoint would refuse.
>
> **Slice 3 (2026-09-21) added the first projection and the first route, and strengthened the grep into a STRUCTURAL check.** `src/modules/work/portal.ts` holds `listPortalTasks()` — the client-visible task list — and `/portal` renders it. The name-list grep stays, but `src/authz/portal-projections.test.ts` now also parses every `portal.ts`/`portal-writes.ts` with the TypeScript AST and fails a read that is not an explicit allow-list: no select-less `find*`, no `include`, no `omit`, no raw SQL, no select that is not an inline literal, and — the one it exists for — **no relation picked whole**. `select: { project: true }` returns every column of the related row while looking exactly like a scalar pick and naming no forbidden column; the check resolves each key against `prisma/schema.prisma` and requires a relation to carry its own select. This is the decision memo's §2.2 ask, and the reason for it is that a forbidden-column list cannot know about an internal column somebody adds next year.
>
> **Slice 4 (2026-09-21) put the member side of this section on a screen, and found that the tab it added was gated on the wrong half of §5.** Project → Portal holds the master switch and a preview that renders `listPortalTasks` through a SYNTHESISED contact principal — the pins' "View as client calls the very same functions", now asserted by `src/authz/portal-view-as.test.ts`, which also pins the CLOSED SET of files that may build a `PortalPrincipal` (two — and since slice 5 they are `requirePortalContext()` on the contact plane and `synthesiseContactPrincipal()` on the member plane, the preview having been moved onto the shared builder rather than the set being widened to three). Three things the slice settles:
>
> * **`project:manage_portal` is in the catalogue and enforced** (TV5, catalog → 98). It replaces `project:edit` on `setPortalEnabled` and `setHoursSharingMode`, which NARROWS both from C M E to C M.
> * **A module-gated surface is hidden with `hasAccess`, never `isAuthorized`.** `isAuthorized` runs gate 4 only, so the first cut of the Portal tab stayed lit for a tenant whose plan or preference had the portal off and walked the member into an error boundary — `requireAccess` denies `DISABLED_BY_TENANT` before the page renders anything. `hasAccess` (`src/entitlements/resolver.ts`) is `requireAccess` as a boolean, and §3.1 of UI.md ("module-gated items hidden, not disabled") means that one and not the other. Not for ✦ codes: `MFA_REQUIRED` is an `AuthzError` too, so a step-up surface asked about this way would vanish instead of prompting. *(2026-09-25)* **For many codes at once, `accessibleCodes`** — `hasAccess` per code, from one read of the member's permissions and up to three of the module gates (`openModules`, module-private), held to `hasAccess` by `access.test.ts`. The member shell's rail uses it: until then the rail asked the permission gate alone, so Time, Files and the `C` key stayed lit for a tenant that had switched their module off.
> * **The member plane owns the REASON the portal plane must hide.** `portalReadOrNull` makes every refusal identical because a reason is a fact about the agency — and the member IS the agency, so the Portal tab lists every blocker that applies (module off, switch off, project archived, no audience), computed member-side from rows they may read anyway, never from the projection's denial. The two never share a code path.

> **Slice 5 (2026-09-21) shipped View-as-Contact, and the load-bearing decision is one this section did not previously have to take: it spans a CLIENT.** What a contact sees is their PORTAL — every shared task of every portal-enabled project their client has — so a project-narrowed view could never be byte-identical to anything a contact receives, and byte-identity is the guard the founder's session-model choice rests on. That moves the whole weight onto the scope check, because `project:manage_portal` is scoped and a member can be assigned to a single project of a client. Entry therefore runs **`assertInScope(tx, actor, { clientId })` with `lifted` left off** — the arm that takes a DIRECT client assignment or `client:view_all` and refuses the project→client lift, which is the exact predicate "this member already reaches everything this mode will show" **on the SCOPE axis**. It is deliberately not a claim about the PERMISSION axis, and both reviews of the slice said so independently: `listPortalTasks` runs under the contact principal and consults no member permission, so a custom role holding `project:manage_portal` plus a direct client assignment — but with `work_item:view` tombstoned — reads that client's CLIENT_VISIBLE task titles, dates and milestone names through this route. Unreachable with seeded templates (C and M hold both), and everything it shows is data the agency itself published to that client, so it is recorded here rather than closed — the same disposition PLAN §0 carries for slice 4's contact-PII-on-`manage_portal` item. The gate is unreachable with seeded templates (`project:manage_portal` is C M, `client:view_all` is C M A, so every template holding the first holds the second), so `view-as.dbtest.ts` builds a custom role to measure it. A second pairing check was found by that dbtest rather than by reading: an owner holds `client:view_all`, so the entry project and the contact could belong to DIFFERENT clients and the audit row would have described nothing that happened; the project must now belong to the contact's own client.

> **~~Also owed and deliberately not done here: the portal plane writes no audit rows yet.~~ CLOSED 2026-09-23 by the invite flow.** The member and platform instances each carry an `AuthAuditSink` and the portal instance carried none, so `auth.login_succeeded` / `auth.login_failed` did not fire for a contact — safe only because no contact could sign in, and owed with or before the first path that could activate one. That path is the invitation flow: `auditPlugin(portalAuditSink)` landed with its SERVER half (`src/auth/portal-audit.ts`), and its SURFACES landed the day after, at which point a contact can really sign in — through `/portal/invite/[token]` and through `/portal/login`, which stopped being a locked shell in the same commit.
...
| `portal.work_item.act` *(added 2026-08-16; **amended 2026-09-22, founder decision — Phase 3 slice 6c**)* | Say their part of an item **assigned to this contact** (`assigneeContactId = principal`) is done, and take it back — **brokered**, a toggle on `work_item.contact_completed_at`; nothing else. ~~**brokered** state change to the project's DONE-category state~~ — **it moves NOTHING**, and the change is the point rather than an implementation detail: the seeded `DONE` state carries `requiresApproval`, so the original wording let the least-trusted principal in the product enter a state an **employee** cannot (`work_item:approve` is C M A). `DONE` now means one thing for everyone — work the agency has accepted — and a client's tick is a CLAIM: a fact about what the client says, not about the state of the work. The agency is notified (`work_item.completed_by_contact`, INSTANT) and closes it themselves. The claim is cleared by the state machine on ARRIVING at a DONE or CANCELLED category, and by any writer that changes the assignment it answers — `work_item_contact_completed_has_assignee` refuses the row that would be left behind. Its resource ref is the **work item** (`PortalScopeRef`, paid by this slice): `work_item`'s `portal_gate` carries the row's own `visibility`, so the ref proves the contact may read that exact task. | v1 (P3) |
...
| `portal.update.view` *(added 2026-08-16)* | Read `PUBLISHED` + `CLIENT_VISIBLE` `ProjectUpdate`s and their `portalSnapshot` (never the internal snapshot, never drafts) — *since 2026-09-25 (Phase 3 slice 67): the snapshot's **hours block** — hours, billed amount, budget — is projected only to a contact who ALSO holds `portal.hours.view`; a collaborator reads the post without it (`listPortalUpdates`)* | v1 (P3) |
...
**Implemented since 2026-09-20 as `authorizePortal(tx, principal, capability, ref?)` in `src/portal/authorize.ts`,** where `tx` is a transaction already running under the contact principal (`withPortalRead`). Two notes on the table above, from building it: `portal.report.view` is marked v2 and is therefore **absent from the union in code** — `CONTACT_PRIMARY` is derived as "every capability", so listing a capability early grants it early; and the two profile bundles are pinned by an exact-list test (`capabilities.test.ts`) precisely because that derivation would otherwise make an addition a silent grant.
...
2. **Brokered writes** — everything else a contact can cause: `portal.request.create`, `portal.work_item.act`, `portal.credential.submit`. The route handler runs `authorizePortal(contact, capability, resource)` **first** (profile, client/tenant ownership, `CLIENT_VISIBLE`, `portalEnabled`, module gates), then performs the write under **`withTenant(tenantId, {type:'system'})`** in `src/modules/<key>/portal-writes.ts`, with hard-coded field values (kind/source/visibility/state/author) that the request body cannot override, an `AuditEvent` naming the contact as actor, and `notify.emit()` in the same tx. `withPlatform` is unreachable from portal routes (ESLint import boundary).

   *Amended 2026-09-21 (Phase 3 slice 6a — the first brokered write ships: portal REQUEST intake).* Four things this paragraph asked for turned out to need a mechanism rather than a discipline, and each is now one:

   - **The file is `portal-writes.ts`, not `portal.ts`** (founder decision, 2026-09-20), so that "this code runs as system" is a property of a filename. `src/authz/portal-projections.test.ts` scans it in BOTH tiers like any portal surface, and `src/portal/brokered-writes.test.ts` pins the shape: every `withTenant` in such a file takes an inline `{type:'system'}`, and `authorizePortal` is called before it opens — an ORDERING assertion on the function's own AST body, never a whole-file grep.
   - **"An `AuditEvent` naming the contact as actor" needed a field.** `record()` derives `actorType` from the transaction's principal, which under a brokered write is `system` — so the row would have read SYSTEM with a null actor in the one event family whose actor is not an employee. `AuditInput.brokeredForContactId` names the contact, and it **throws outside a system transaction**: a member passing it would be attributing their own act to a client, in the one table SECURITY.md §7 treats as evidentiary.
   - **The rate limit is a Postgres count, not `src/ratelimit`.** That limiter fails OPEN when Upstash is unconfigured, which it is and is documented to be, so keying the intake on it would have shipped a control that does nothing in the only deployment that exists. `assertRequestBudget` counts the contact's own PORTAL-sourced rows inside a window, behind a per-contact advisory lock so two simultaneous submits cannot both see N−1, inside the writer's own transaction. Its own header records the choice.
   - **A denial's REASON is not the contact's to have, on writes either.** `portalReadOrNull` made every read refusal identical; `src/portal/action.ts` does the same for server actions — every `AuthzError` is one message and the reason goes to the server log, and a `DomainError` is **allow-listed** rather than passed through (today: `REQUEST_RATE_LIMITED` and `INVALID_INPUT`, both facts about the reader). The allow-list is enumerated against every code the product has copy for in `src/portal/action.test.ts`.

   *And a correction a code review had to make to the paragraph above (2026-09-21):* the sentence originally said the tripwire scanned "it" — true of `portal-writes.ts`, false of `src/modules/work/requests.ts`, which the split moved the actual INSERT into and which matched **neither** tier. It is now in the STRUCTURAL tier and deliberately not the TEXT one (its prose necessarily spells `stateId`), exactly as `view-as` already is, and the widening is itself pinned. The guarantee is therefore: *every select on the intake path is an explicit allow-list, checked by AST; the forbidden-NAME grep covers the broker and not the row-shaping beside it.* What crosses between them is pinned as a closed five-field shape by `brokered-writes.test.ts`, so a contact-facing field appearing there is a visible change rather than a silent one.

   *Also settled by building it:* the intake row is the one row in the product **born `CLIENT_VISIBLE`** — a request the submitter cannot read back is a form that ate their words — and the row's INSERT lives in `src/modules/work/requests.ts` rather than inline in the broker, because the forbidden-columns grep covers a `portal-writes.ts` file's text and a create that lands a row in a workflow state has to name `stateId`. Keeping that list absolute was judged worth more than the inlining — and what the client reads back from that row is the TITLE, not the body: `listPortalTasks` never projects `descriptionText`.

Both shapes are enforced by two CI tripwires that ship in the same commit as each portal feature: the **forbidden-columns grep** over `modules/*/portal.ts` (rates, cost, `internalNotes`, `repoUrl`, `hostingNotes`, non-billable, per-member breakdown, state names, labels, links, `assigneeMemberId`, INTERNAL activity, `Contact.invitedById` — a member id on a row every contact of the client may read — ciphertext) and the **contact-writable census test** (any new `INSERT`/`UPDATE` policy for the portal role outside the census fails the build). Adding a third write shape — e.g. "the contact edits its own request" — means extending the census and the test, never widening a policy quietly. "View as client" (`project:manage_portal`) calls the very same `portal.ts` functions under a synthetic contact principal, asserted by an import-graph test.
