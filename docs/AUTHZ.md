...
| `project:manage_portal` | portal | `Project.portalEnabled`, `hoursSharingMode`, task-list/kanban toggles, "View as client" — audited *(the "audited" binds the two WRITES, which record `project.portal_enabled` / `portal_disabled` / `hours_sharing_changed`, and the explicit **View-as-Contact** mode, which records `project.viewed_as_contact`. The Portal tab's "what the client sees" panel, shipped 2026-09-21, records NOTHING: it is a page RENDER, re-run on prefetch and revalidation, over CLIENT_VISIBLE rows of a project the member already holds `project:view` and scope on. Founder disposition wanted — PLAN §0.)* | C M | scoped | 3 (TV5, **landed 2026-09-21**) |
...
>
> **Slice 4 (2026-09-21) put the member side of this section on a screen, and found that the tab it added was gated on the wrong half of §5.** Project → Portal holds the master switch and a preview that renders `listPortalTasks` through a SYNTHESISED contact principal — the pins' "View as client calls the very same functions", now asserted by `src/authz/portal-view-as.test.ts`, which also pins the CLOSED SET of files that may build a `PortalPrincipal` (two: `requirePortalContext()` and the preview). Three things the slice settles:
>
