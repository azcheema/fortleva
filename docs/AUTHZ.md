...
>
> **Denial reasons are internal.** `authorizePortal()` throws `AuthzError` with a precise reason for the audit row, the server log and the test matrix. The portal's HTTP surface must render every one of them identically — a contact told `NOT_ENTITLED` has been told something about their agency's commercial arrangements. Owed by the first portal route (slice 3).
>
