...
| `work_item:triage` | work | Accept / Decline / Duplicate / Snooze a `REQUEST` out of `TRIAGE` — **enforced 2026-09-22**, `src/modules/work/triage.ts`. Its own code and not a flavour of `work_item:edit`: accepting work commits the agency to it and declining speaks to a client in the agency's name. An `edit`-only member can still drag a request into "To do" (that is accepting it) but cannot end one | C M E | scoped | 2W |
