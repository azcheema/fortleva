...
>
> Status legend: `[ ]` not started · `[~]` in progress · `[x]` done · `[-]` superseded (kept for history, dated). Last updated: **2026-08-16** (Phase 1b close-out; earlier same day: work-management plan reconciled — decisions 11–13, phases 1b / 2W / 2T / 3V added; previous: 2026-08-03, Phase 0).
>
...
| 1b | Foundation close-out *(added 2026-08-16)* | i18n, UI kit, MFA enforcement, member admin, R2 transport, crypto v2, GUC/registry/counter helpers — every screen after this assumes them | 2–3 wk | `[~]` code complete 2026-08-16 (commits `a9a5014`…`5c7d971` + close-out); open: Upstash limiter, Neon spike, staff-notice seed, RUNBOOK skeleton, founder provisioning |
...
- [ ] **Upstash rate limiting** behind one config module with a no-op (fail-open) fallback; vault reveal budget later uses a fail-closed Postgres counter (3V). *(open — code not started; the no-op module can be built before the Upstash env exists. Until then Better Auth's built-in limiter guards the auth routes.)*
...
- [~] **Model-registry subclasses** (`clientScoped`, `projectScoped`) + the **registry posture test** (§2 tripwire) — `a9a5014` (`RLS_CLASSES` registry, `tableNameOf`, posture dbtest; no-GUC / contact-denied tests iterate the registry); `withPlatform` import-boundary ESLint rule + test — **open** (only the one-seam rule and the literal-strings rule exist; ARC-16 module boundaries not yet linted).
...
- [x] `Member.timezone`, `Member.workCountry`, `Member.hoursPerDay?` (`DATA_MODEL.md` amendment). — `a9a5014` (columns exist; `request.ts` still pins Europe/Stockholm — wiring `Member.timezone` into formatting is a 2T item).
...
- [ ] `docs/RUNBOOK.md` skeleton (self-hosting; kept current every phase). *(open)*
- [ ] Vercel Pro (execution item, already ARC-01) — founder.

**Tests:** MFA deny-matrix; last-owner guard; audit rows carry `requestId`; storage transport contract (local always; R2 when env exists); v2 round-trip + AAD mismatch fails + v1 decrypts; i18n lint; registry posture; import-boundary. — **All green at close-out except import-boundary (not written).** Suite at close-out: unit 163 pass / 1 skipped (R2), db 101 pass, typecheck, eslint `--max-warnings 0`, `next build` clean.
