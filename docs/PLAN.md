...
| ESP selection + DNS (SPF/DKIM/DMARC on the v1 sending subdomain under `naxdor.com`) and sender warm-up | **now — blocks Phase 1's invite flow** (`OPEN_QUESTIONS.md` B4); warm-up is calendar time, not effort | Phase 1 week 1–2 |
...
Scope: this document set. Exit criteria: founder reviews the 8 docs and resolves the **blocks-Phase-1** group in `OPEN_QUESTIONS.md` — after decision 9 that group is **B2, B3, B4 and B6**:
...
- **B2** Neon project created in Frankfurt `aws-eu-central-1`;
- **B3** template-drift policy (shapes the `Role`/`RolePermission` migration);
- **B4** ESP choice — **blocks the invite flow**, needed by Phase 1 week 1–2 (DNS + warm-up lead time);
- **B5** pricing currency (flagged one-way door; decide by Phase 7 design start, does not gate Phase 1 code);
- **B6** default role-seeding matrix sign-off (`AUTHZ.md` §3.2 C/M/A/E columns) before the Phase-1 seed migration.
...
- [ ] i18n scaffold (sv + en); transactional email adapter (invites); baseline auth rate limiting (Upstash Redis EU).
