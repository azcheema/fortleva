...
| Platform auth | `platform.login_succeeded`, `platform.login_failed`, `platform.mfa_verification_failed`, `platform.mfa_enabled`, `platform.mfa_disabled`, `platform.password_changed`, `platform.email_changed` [PLATFORM] — **added 2026-09-11.** The ops console recorded nothing at all before that: its Better Auth instance carried no `auditPlugin` and `databaseHooks` holding only the session stamp, so sign-ins, failures and second-factor changes on the plane that reaches `app_platform` left no trace. They are `platform.*` rather than `auth.*` because they are a different KIND of row — `tenant_id` is NULL (there is no tenant, not an unknown one), the actor id lives in the global `user` namespace rather than a tenant's `member` namespace, and the audience is the operator. Writing them as `auth.*` would also route them through `recordForUserMemberships`, which fans out per ACTIVE membership and therefore writes ZERO rows, silently, for a platform admin who is a member of no tenant. Written by `recordPlatformEvent()` (`src/db/platform-audit.ts`) under `app_platform`, because `audit_tenant_insert`'s `WITH CHECK (tenant_id = app.tenant_id)` forbids `app_runtime` from inserting a NULL tenant at all. |
...
| Contacts & portal | `contact.created`, `contact.invited`, `contact.activated`, `contact.suspended`, `contact.access_restored`, `contact.access_revoked`, `contact.updated`, `contact.deleted` [TENANT] |
...
/// audit: contact.created | contact.invited | contact.activated | contact.suspended | contact.access_restored | contact.access_revoked
///
...
**Portal projection (`modules/work/portal.ts`).** Allow-listed select. ~~`number, title, stateCategory, targetDate, milestoneId, assigneeContactId, visibility, updatedAt`~~ — **the draft list above was written before the projection existed and what shipped on 2026-09-21 (Phase 3 slice 3, `listPortalTasks`) is NARROWER, which a security review flagged as a document/implementation disagreement worth closing in the same commit.** What ships: `id, title, stateCategory, targetDate, completedAt` + the project's `id`/`name` + the milestone's `name`. Four differences and each is deliberate. **`number` is out**: the per-project counter tells a client how many items the project holds, including the ones they cannot see. **`milestoneId` became the milestone's NAME**, resolved by the same contact-principal read, so an INTERNAL milestone comes back null instead of handing over an id the client could correlate. **`visibility` and `updatedAt` are out**: every row on this surface is CLIENT_VISIBLE by construction, so the column says nothing, and a modification time exposes internal churn on a row whose content did not change. **`completedAt` is in**, which the draft did not have: it is only ever set on a row the client already sees as DONE, and "when was it finished" is the question a portal exists to answer without an email. CLIENT_VISIBLE comments and activity are still owed by their own slices. **Never:** state names, labels, links, priority, estimates, `rank` (position IS importance, §3.1), `assigneeMemberId`, INTERNAL rows. **The projection also filters an ARCHIVED project, and the founder confirmed that on 2026-09-21** (`project`'s `portal_gate` binds client + `portal_enabled` and has no archive term, and `archiveProject` leaves `portalEnabled` TRUE, so without this an agency that finished a project and moved on went on publishing its task list for ever). It is a projection decision, so **every portal projection that follows must repeat it** — there is no policy doing this for them. Reversible: unarchiving un-hides, nothing is written. *2026-09-21:* the function also takes an optional `{ projectId }`, which narrows the `where` AND names the project to `authorizePortal` — so a narrowed read is refused outright for a project whose portal switch is off, where the unnarrowed one merely filters its rows. Assignee (member) names are not shown in the portal in v1 (P10). A forbidden-columns grep test and the "no INTERNAL fact to a Contact" fixture suite ship with the feature. View-as-Contact calls the same functions (asserted by import graph).
...
**Landed 2026-09-25 (Phase 3 slice 67 — migration `20260925200000_project_update`, `src/modules/work/updates.ts`, `update-metrics.ts`, `update-snapshot.ts`, `update-body.ts`).** The block above is built as written, with these readings where it left room:

- **`seq` is allocated at PUBLISH**, not at draft: `seq Int?`, `CHECK ((status = 'DRAFT') = (seq IS NULL))`, and the `(tenantId, projectId, seq)` unique treats NULLs as distinct. The counter never runs backwards — a retracted post's number is spent.
- **A draft is INTERNAL by CHECK** (`project_update_visible_only_published`: `visibility = 'INTERNAL' OR status = 'PUBLISHED'`), so the audience is chosen in the publish dialog, never stored as intent on a row nobody has published; **archiving sets INTERNAL** for the same reason. The portal gate is the four-term form (`+ status = 'PUBLISHED'`, `time_report`'s shape).
- **The fifteen-minute grace window is a RETRACTION, enforced twice.** The trigger's one exception to immutability: a PUBLISHED row may go back to DRAFT within fifteen minutes of `published_at`, provided it gives up `seq`, `published_at`, `published_by_member_id`, both snapshots and its audience (`visibility = 'INTERNAL'`); the service (`retractUpdate`, `project_update:publish`) applies the same clock and deletes the class-A twin. After the window the only text that changes is `editNote` — the annotation under an immutable post (`annotateUpdate`, audited `project_update.annotated`, client-readable when the post is). The "re-issue as a new seq" reading is therefore simply "publish a new post".
- **`body.metrics.include`** — the composer's "metrics card with include toggles" — rides in `body` (`{ sections, metrics: { include: { tasks, milestones, versions, requests, hours } } }`) rather than a column; a group left out is absent from `portalSnapshot`, never present-and-empty. The snapshot's allow-list is `PORTAL_SNAPSHOT_KEYS` (`update-snapshot.ts`), pinned by `update-metrics.test.ts`, and `updates.dbtest.ts` walks a real one for the fixture's sentinels (an INTERNAL task's title, an INTERNAL milestone's name, the member's display name and id).
- **The internal snapshot is read in parts — and so is the portal snapshot's hours block, on BOTH planes** (the slice's security review). `getUpdate` hands back `byMember` only with `time:view_team`, `budget` only with `budget:view` (amount only with `rate:view_bill`), `cost` only with `rate:view_cost` ✦ and a recent factor — the codes that gate each figure's live source (AUTHZ.md §3.1 amended); and `redactHoursFor` (`update-snapshot.ts`) applies the same three codes to the frozen `hours` block a member reads off `getUpdate` and previews in `readComposerContext`, so a frozen post never widens what the Time tab, the budget card or the Money page would show them live. On the portal, `listPortalUpdates` asks `portal.hours.view` — `CONTACT_PRIMARY` only, riding on `time` (AUTHZ.md §8) — without throwing and projects the post WITHOUT the block when it is refused: a collaborator reads the words and the task counts, never the hours or the money. The row itself stays complete: it is what the agency shared with the client company. At publish, cost is computed only when the publisher can reveal it (`revealCostRates`, the audited ✦ act, outside the publishing transaction — `projectMoney`'s shape); a stale factor freezes no cost and never turns a publish into a step-up.
- **Publish is a compare-and-set on the draft — in the WRITE.** The first transaction reads the draft (`updatedAt` included) and decides what this publisher may freeze; the third freezes with an `updateMany` bound to `status = 'DRAFT' AND updatedAt = <as read>`, and a zero-row write is `UPDATE_CHANGED` — a draft another member saved in between is never published with a window, toggles or a body the publisher did not see. (The slice's code review asked for the check; the review of the fix noticed a check a dozen round trips before an unconditional write is not one, so the condition moved into the write.)
- **The reader's figure codes are resolved on all four gates** (`accessibleCodes`, one call), not the permission alone: the Time tab, the budget card and the Money page go through `requireAccess`, so a tenant that switched `time` off finds the frozen figures gone from a post as well — on both planes, since the portal's `portal.hours.view` verdict already includes the module.
- **Drafting is audited once** (`project_update.drafted`; discarding, `project_update.draft_discarded`); a draft's EDITS are routine — AGENTS.md's carve-out, extended.
- **"In period" is one rule in one place** (`metricsWindowFor`): the author's dates as whole local days in the tenant's zone, end day inclusive; else from the previous published post (the day after its period, or its publishing instant) to now; else from the project's creation. The composer's metrics card, its pull-in panel and the freeze at publish all use it.
- **Requests count `open` (TRIAGE) and `acceptedInPeriod` (`acceptedAt`, C31)** — not "closed", because a decline carries no timestamp of its own and the snapshot states only what a column records.
- **Task counts are the client's list's counts**: CLIENT_VISIBLE, live, not a request, not cancelled — a number that disagreed with the list under it would be a number the client could not trust.
- **`pdfDocumentId` is reserved and unwritten** (the post-publish PDF is a later slice); **no search feed yet** (`bodyText` is extracted; the `search_index` trigger and the palette's entity type land with a search slice); **no notification kind yet** (a client digest is Phase 5; nothing is emitted on publish).
- **Deleting a published row needs the transaction-local `app.work_maintenance` GUC** (`app.time_maintenance`'s shape) — set by test teardown and the e2e harness's `removeTenant`, never by a tenant-plane service.
...
*Landed 2026-09-26 (Phase 3 slice 68 — `listPortalTimeline`, `src/modules/work/portal.ts`; the header's `readPortalProjectSummary`, `src/projects/portal.ts`).* Three branches as written — updates, milestone completions and dues, version ships — as four sequential Prisma reads merged in the service rather than a SQL `UNION ALL`, so each `select` is an allow-list the structural tripwire can read; newest first, a future due date above everything past, cut at 100 after the merge. The two branches over `FileVersion` and over approval decisions are NOT here yet and the reason is this section's own: `FileVersion` is class A (§1.4 — "the portal never queries the file layer"), so a version event needs the files slice's brokered read; and no approval decision has a writer until the sign-off slice. Cancelled milestones are excluded from the rail and from the header's count (the snapshot's rule above); a milestone's status is not projected at all — the header names the one phase in progress.
...
**Amended 2026-09-06 (the `/inbox` + preferences slice — four things settled by building it):**

1. **A TEMPLATE KEY IS NOT A NOTIFICATION KIND.** `EmailOutbox.kind` was always documented as a *template key*, but the worker resolved it through `isNotificationKind()` and marked anything else DEAD. Mail exists that no fan-out produces — the 2T weekly self-reminder (D6), addressed to one person who asked for it, with no actor, no entity and no inbox row — so `src/notify/templates.ts` now owns the template registry (`EmailTemplateKey` = every `NotificationKind` **plus** the extras) and `isEmailTemplate()` is what the outbox checks. A kind still may not carry email behaviour without being INSTANT; a template may exist without a kind.
2. **`emailLevel` is a LADDER, honoured per kind.** Each emailing kind declares `email.atLevel` — the weakest level that still receives it (`comment.mentioned` → MENTIONS, `work_item.assigned` → PARTICIPATING) — and `emailAllowed(level, kind)` compares ranks `NONE < MENTIONS < PARTICIPATING < ALL`. `atLevel: "NONE"` is **not expressible in the type**: a level of NONE means no mail ever, and a kind that could override it would make the member's setting a suggestion. A receiver with no preference row gets the schema default PARTICIPATING, so an unwritten row and a written default are the same answer. **The in-app row is never gated** — an assignment you cannot see is work you never find out about.
3. **`perKind` is the extension point, and the weekly reminder uses it.** The 2T D6 opt-in lives at `perKind["time.weekly_reminder"].email === true` — no column, no migration. It is OPT-IN: absent, `{}`, `false` and any truthy-but-not-`true` value all mean off. Writes MERGE into whatever `perKind` holds, so a setting a newer deploy wrote is never dropped by an older one.
4. **`Notification.id` must be UUIDv7** (`notify.emit` calls `newId()`, not `randomUUID()`), because `/inbox` pages on it. `created_at` is `timestamptz(6)` defaulted to `now()` — transaction start, to the microsecond — and Prisma hands it back as a millisecond `Date`, so a cursor built from one skips every row stamped inside that millisecond. Rows emitted in one transaction share the stamp exactly, so a batch straddling a page boundary vanished. The id is the only total order that round-trips.
5. **A kind's `audience` names who RECEIVES, not who CAUSED it** *(added 2026-09-21, Phase 3 slice 6a)*. `work_item.request_received` is the first kind where the two differ: a CONTACT submits the request and MEMBERS are told. It is therefore a MEMBER kind and must NOT carry `clientVisibleOnly` — that flag is required on every CONTACT-audience kind and claims the fan-out runs from a client-visible fact for a client's benefit, which is the opposite of what this one does. It is the **third INSTANT kind**, after assignment and mention: a request is the only thing a client can put on an agency's board, and an agency that learns about it in Friday's digest has a client who was ignored all week. It mails at PARTICIPATING and never at MENTIONS, because it names nobody. Receivers are the project's `MemberProject` members plus its lead, ACTIVE only; a tenant whose members reach projects through a tenant-wide role has none, so the request lands in triage with nobody told — recorded as owed rather than papered over, because widening it means resolving scope for a whole tenant inside an intake transaction. The notification carries **no actor**: `emit` only knows how to name a member.
