...
`pnpm dev` · `pnpm build` · `pnpm typecheck` · `pnpm exec eslint src e2e --max-warnings 0` · `pnpm test` (unit) · `pnpm test:db` (against Neon, sequential, **~14 min** — measured 2026-09-02 at 27 files / 288 tests; the "~3 min" this used to say dated from ~20 files and is what makes a 10-minute timeout kill a healthy run) · `pnpm test:e2e` (Playwright; `pnpm exec playwright test visual` regenerates 172 screenshots — 43 stops × light/dark × desktop/phone, written locally only (CI skips the file write) — into `.design-shots/`).
