...
`pnpm dev` · `pnpm build` · `pnpm typecheck` · `pnpm exec eslint src e2e --max-warnings 0` · `pnpm test` (unit) · `pnpm test:db` (against Neon, sequential, ~3 min) · `pnpm test:e2e` (Playwright; `pnpm exec playwright test visual` regenerates 172 screenshots — 43 stops × light/dark × desktop/phone, written locally only (CI skips the file write) — into `.design-shots/`).
